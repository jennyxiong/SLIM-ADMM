...
class ADMM:
    ...
    def compute_recommendation(self, lambda2= 500, lambda1= 1, rho=10000, alpha=0.5, lam_bda=0.02, max_iter=5, tol=0.0001, N=10, adapting_rho=False,parallel = False):
        """
        开始计算推荐列表
    ...
    def admm(self):
        ...
        while True: 
          ...
          e_primal = 1600*e_abs + e_rel * max(numpy.linalg.norm(B,ord="fro"),numpy.linalg.norm(C,ord="fro"))
          e_dual = 1600*e_abs + e_rel * numpy.linalg.norm(Gamma,ord="fro")    
          r_primal = B-C
    ...
    def admm_parallel(self,shape,X,P,start,end):
        #print(start,end)
        lambda2 = self.lambda2
        lambda1 = self.lambda1
        rho = self.rho
    ...
    def __aggregation_coefficients(self,lambda2= 500, lambda1= 1, rho=10000, alpha=0.5, lam_bda=0.02, max_iter=5):
        ...
        with ProcessPoolExecutor() as executor: #default: 2 workers at the same time
          return numpy.hstack(executor.map(self.admm_parallel, [shape]*n,[X]*n,[P]*n,starts, ends))
...
if __name__ == '__main__':
    algorithms = [ADMM] #put the selected algorithm into the list to run them: UserCF, ItemCF, LFM, SLIM
    precisions = []
    recalls = []
    coverages = []
    popularities = []
    ...
    for algorithm in algorithms:
        ...
        ##change the parameter in the next line
        recommend.compute_recommendation(lambda2= 500, lambda1= 1, rho=10000, max_iter=20, adapting_rho=False, parallel = False) 
